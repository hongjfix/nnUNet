...
if __name__ == "__main__":
    ...
    for p in subdirs(cur, join=False):
        ...
        shutil.copy(t1, join(target_imagesTr,
                             p + "_0000.nii.gz"))
        shutil.copy(t1c, join(target_imagesTr,
                              p + "_0001.nii.gz"))
        shutil.copy(t2, join(target_imagesTr,
                             p + "_0002.nii.gz"))
        shutil.copy(flair, join(target_imagesTr,
                                p + "_0003.nii.gz"))

        copy_BraTS_segmentation_and_convert_labels(
            seg, join(target_labelsTr, p + ".nii.gz"))


    json_dict = OrderedDict()
    json_dict['name'] = "FeTS2021"
    json_dict['description'] = "nothing"
    json_dict['tensorImageSize'] = "4D"
    json_dict['reference'] = "see FeTS2021"
    json_dict['licence'] = "see FeTS2021 license"
